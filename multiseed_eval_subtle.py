#!/usr/bin/env python3
"""
Trimmed rerun for the peer-review significance-test request.

Runs ONLY:
  - Anomaly types: freeze_zero, freeze_last_value
    (the two "subtle" types carrying the paper's attention-advantage claim
    -- spike/step both hit AUC-ROC ceiling 1.000 for both models and are
    not in dispute, so they're skipped here to save compute.)
  - Models: AE-LSTM-ATT vs LSTM-AE (the attention ablation pair the
    reviewers are asking about). Isolation Forest is skipped -- it's not
    part of the attention-vs-no-attention comparison.

Everything else (config, seeds, splits, threshold protocol) is IDENTICAL
to your original multiseed_eval.py, so results are directly comparable to
Table 3 in the manuscript.

Saves results_raw.json with the per-seed values needed for the paired
Wilcoxon / paired t-test requested by the reviewers -- run
paired_significance_test.py on the output afterward.

REQUIRES: ae_lstm_att.py and Dataset.py (your real files) in the same
directory. Run on a GPU environment (Colab/Kaggle) -- CPU-only will be
very slow for the autoregressive decoder.

SET the data path via INDRAD_DATA_PATH, e.g.:
...
try:
    ...
    RESULTS_DIR = "/content/drive/MyDrive/ae_lstm_att_results_subtle"
    print(f"Running on Colab -- results will be saved to Google Drive: {RESULTS_DIR}")
except Exception:
    RESULTS_DIR = "results_multiseed_subtle"
...
# ---- config, IDENTICAL to the original multiseed_eval.py ----
WINDOW_SIZE   = 50
HIDDEN_SIZE   = 64
NUM_LAYERS    = 2
DROPOUT       = 0.3
...
ANOMALY_TYPES = ["freeze_zero", "freeze_last_value"]   # <-- trimmed
...
class LSTMAEBaseline(nn.Module):
    """Same structure as in multiseed_eval.py: matched encoder/decoder,
    no attention context vector -- isolates attention's effect."""
...
if __name__ == "__main__":
    ...
    for seed in SEEDS:
        ...
        try:
            ...
            print(f"!!! Re-running the script will resume from the last checkpoint "
                  f"({sorted(completed_seeds)}) and retry this seed.")
            continue
    ...
    summary_df.to_csv(f"{RESULTS_DIR}/results_meanCI_subtle.csv", index=False)
    with open(f"{RESULTS_DIR}/results_raw.json", "w") as f:
        json.dump(all_results, f, indent=2, default=float)

    print("\n=== FINAL: mean +/- 95% CI across", len(SEEDS), "seeds ===")
    print(summary_df.to_string(index=False))
    print(f"\nSaved: {RESULTS_DIR}/results_meanCI_subtle.csv and {RESULTS_DIR}/results_raw.json")
    print("\nNext: run paired_significance_test.py on results_raw.json")
